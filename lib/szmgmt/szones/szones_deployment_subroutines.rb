    ...
    class SZONESDeploymentSubroutines
      ...
      def self.deploy_zone_from_files(zone_name, path_to_zonecfg, opts = {}, ssh = nil, host_spec = {:host_name => 'localhost'})
        ...
        if opts[:zonepath]
          ...
          SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -      Zonepath of zone #{zone_name} adjusted.")
        end
      ...
      def self.deploy_zone_from_local_zone(zone_name, source_zone_name, opts = {}, ssh = nil, host_spec = {:host_name => 'localhost'} )
        cleaner = opts[:cleaner]
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -      Configuring zone #{zone_name} from srouce zone #{source_zone_name}...")
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -             Cloning configuration from #{source_zone_name}...")
        ...
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -      Cloning zone #{zone_name} from source zone #{source_zone_name}...")
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -              type: clone.")
        cleaner.add_persistent_zone_installation(zone_name, host_spec)
        clone = SZONESBasicZoneCommands.clone_zone(zone_name, source_zone_name)
        ssh ? clone.exec_ssh(ssh) : clone.exec
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -      Cloning zone #{zone_name} from source zone #{source_zone_name} finished.")
      end

      def self.deploy_zone_from_uar(zone_name, path_to_uar, opts = {}, ssh = nil, host_spec = {:host_name => 'localhost'})
        cleaner = opts[:cleaner]
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -      Configuring zone #{zone_name} from UAR...")
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -                uar: #{path_to_uar}...")
        ...
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -      Installing zone #{zone_name} from UAR...")
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -                uar: #{path_to_uar}...")
        cleaner.add_persistent_zone_installation(zone_name, host_spec)
      ...
      def self.deploy_zone_from_zfs_archive(zone_name, path_to_archive, path_to_zonecfg, opts = {}, ssh = nil, host_spec = {:host_name => 'localhost'})
        cleaner = opts[:cleaner]
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -      Configuring zone #{zone_name} from command file...")
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -            zonecfg: #{path_to_zonecfg}...")
        configure = SZONESBasicZoneCommands.configure_zone_from_file(zone_name, path_to_zonecfg)
        ssh ? configure.exec_ssh(ssh) : configure.exec
        cleaner.add_persistent_zone_configuration(zone_name, host_spec)
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -      Configuration of zone #{zone_name} created.")
        ...
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -      Installing zone #{zone_name} from ZFS archive...")
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -            archive: #{path_to_archive}...")
        cleaner.add_persistent_zone_installation(zone_name, host_spec)
      ...
      def self.boot_zone(zone_name, opts = {}, ssh = nil)
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -      Booting up the zone #{zone_name}")
        boot = SZONESBasicZoneCommands.boot_zone(zone_name)
        ssh ? boot.exec_ssh(ssh) : boot.exec
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -      Zone #{zone_name} booted.")
      end
      ...
      def self.export_zone_to_zfs_archive(zone_name, opts = {}, ssh = nil, remote_host_spec = {:host_name => 'localhost'})
        ...
        cleaner = opts[:cleaner]
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -      Determinig volume name of zone #{zone_name} on host #{remote_host_spec[:host_name]}...")
        volume = SZONESBasicRoutines.determine_zone_volume(zone_name, ssh)
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -      Creating snapshot of #{zone_name} on host #{remote_host_spec[:host_name]}...")
        ...
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -      Creating archive of zone #{zone_name} on host #{remote_host_spec[:host_name]}...")
        ...
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -      Exporting zone #{zone_name} configuration on host #{remote_host_spec[:host_name]}...")
      ...
      def self.export_zone_to_remote_zfs_archive(zone_name, destination_host_spec, opts = {}, ssh = nil, remote_host_spec = {:host_name => 'localhost'})
        ...
        ssh ? snapshot.exec_ssh(ssh) : snapshot.exec
        cleaner.add_tmp_volume("#{volume}@#{snapshot_name}", remote_host_spec)
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -      Creating archive of zone #{zone_name} on host #{destination_host_spec[:host_name]}:#{path_to_archive}...")
        ...
        SZMGMT.logger.info("DEPLOY (#{opts[:id]}) -      Exporting zone #{zone_name} configuration on host #{destination_host_spec[:host_name]}:#{path_to_zonecfg}...")
